//! SQL 前置解析拦截（plan v2.0 §4.3 / operation-log §8.2，S1.6 / S1.7）。
//!
//! **原则**：server 把 SQL 解析为 AST 后按变体分流——只有 SELECT 让 DataFusion 处理；
//! DML / DDL 在节点层直接落到 ingest / catalog 能力（DataFusion 不触碰，避免会话级
//! 副作用与持久化 Catalog 语义冲突）。
//!
//! 技术选型：`sqlparser`（与 DataFusion 同源解析器，版本对齐其依赖 0.62），
//! 方言 MVP 用 `GenericDialect`。本模块只承载**纯函数**（AST → 数据 / 元数据），
//! 分流编排（catalog / ingest / query 调用）在 [`crate::flight::FlightServer`]。
...
pub fn insert_target(sql: &str) -> Option<String> {
    match parse_single(sql) {
        ...
        _ => crate::flight::insert_target_table(sql),
    }
}
...
pub fn show_tables_batch(tables: &[String]) -> RecordBatch {
    ...
    RecordBatch::try_new(
        schema,
        vec![
            Arc::new(StringArray::from(vec![crate::flight::CATALOG_NAME; n])) as ArrayRef,
            Arc::new(StringArray::from(vec![crate::flight::SCHEMA_NAME; n])) as ArrayRef,
