...
//!
//! # ⚠️ 已知风险：这里"丢一条"的代价被低估了（`§106` 复现 / `§107` 定位 / **`§111` 收窄**）
//!
...
//! ## 现在走到哪一步了（**别读成"已修好"**）
//!
//! * **`§111` 修掉的是"我们自己造成的丢包"**：原先一条 FIFO 队列被过时的心跳/空追加堵住队头，
//!   唯一携带条目的那条 append **一次都没出得去**（实测 raft 交出 537 条、`send_loop` 只出队
//!   298 条，那条救场的 append 到达次数 = **0**）。改成**两条队列**（携带条目的 = 重要；
//!   心跳/空追加 = 可取代、容量 1）+ `send_loop` 用 `biased` 优先发重要的之后，那条复现探针
//!   （`compaction_with_lagging_follower_should_keep_committing`）**按纪律转正、常绿**
//!   （`§122` 复核连跑 5/5），全仓因此 **0 ignored**；
//! * **机制本身没有消除**：RPC 真失败（对端不可达 / 超时 / 连接被断）时这条 append 仍会被
//!   丢掉，而 raft **不会**重发 ⇒ **理论上仍可能停摆**（触发条件从"自己堵自己"变成
//!   "真的丢包/超时"）。三种改法都被实测否掉（无界队列 / 送达为止重试 / 每次丢包就
//!   `report_unreachable`），逐条见 `operation-log §107.5–§107.7`；
//! * **要彻底修**得走 raft 自己留的入口 —— `RawNode::report_unreachable`（raft 收到
//!   `MsgUnreachable` 会把该 peer 从 Replicate 退回 Probe ⇒ `next_idx = matched + 1`）——
//!   但必须**限流**（每次丢包都报会把复制限流在 Probe 节奏上），并且要先有能证明
//!   "它真能救回来"的用例。
//!
...
/// leader 永久停摆（见模块文档"已知风险"）。这个深度本身**没有**动（无界队列与"送达为止重试"
/// 都试过，副作用——长期分区下内存无界 / 队头阻塞把新 append 也堵住——都被实测证实）；
/// `§111` 改的是**分流**：心跳/空追加走另一条容量 1 的队列，于是"过时消息堵住重要消息"
/// 这条**自己造成的丢包途径**没有了。
    ...
    loop {
        ...
        match client.raft(req).await {
            ...
            Err(e) => {
                ...
                // leader 永久停摆（模块文档"已知风险"）。`§111` 之后实测探针已常绿（`§122` 复核），
                // 但**机制仍在**：真丢包时这条 append 不会自己回来。
                stats.failed.fetch_add(1, Ordering::Relaxed);
                fail_streak += 1;
