...
| R6 读己之写 | 维持回执方案 | Query 广播拉 MemTable | **维持回执方案**（广播在 100 节点不可行）；**【更新】store 层分片（内存分片 / 磁盘分片）落地后，单机读己之写不再依赖回执等待 —— 见 design §7.4** |
...
| # | 风险 | 缓解 |
|---|---|---|
| 1 | 分离后"读己之写"延迟 = 攒批阈值 | **store 层分片**：内存分片（热）/ 磁盘分片（冷）两级，查询只面向 store 层（design §7.4）。阶段 0 单进程内直接读内存分片（≤1 个攒批扫描周期）；分离后由分片服务提供同形接口（定向拉取，非广播）。回执 `expected_visible_at` 返回真实上界 |
