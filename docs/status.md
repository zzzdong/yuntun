...
- **质量网**：381 用例全绿（**+1 ignored**） / clippy 本仓 0 告警 / chaos **11/11** / T8 基线已入库 /
  ...
  **一条已登记、已复现、根因收窄但未修好**：`§106` 的写停摆（小压缩阈值**且**有 follower 落后时
  集群 30s+ 写不进）。`§107` 已修掉其中**一个真问题**（传输层会**静默丢** raft 消息 ⇒ 改为
  入队无界 + 送达为止重试），并在代码里留下四条诊断轨迹；**停摆本身**仍未修（`§107.4` 有一处
  视图偏差待查）；**默认阈值 10 万不受影响**）。诊断配方：`RUST_LOG=raft=debug` +
  `YUNTUN_META_TRACE=1` 就是 metanode 的全部开关（`§106.1` / `§107.1`）。
...
| `operation-log.md` | 实施日志 + **偏差与证据**（§1–§107） | ✅ 最新 | 每刀一节的纪律保持；§98（M4 并发真写）/ §99（读侧栅栏）/ §100（客户端落点）/ §101（真实 S3 单机读写）/ §102（数据进程接 S3 + 多节点真 S3 对拍）/ §103（3 个真 metanode 进程 + 修掉多节点启动缺陷）/ §104（网络分区：少数派不能提交）/ §105（压缩触发策略）/ §106（写停摆复现）/ §107（**传输不再静默丢 raft 消息** + 四条诊断轨迹） |
