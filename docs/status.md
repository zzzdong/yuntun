...
- **真实对象存储 / 跨机**：只验过本机 `--cold-root`（多进程共享目录）；S3/MinIO、跨机、跨可用区未测。
...
| # | 事项 | 准入 / 验收 | 为什么是这个顺序 |
|---|---|---|---|
| 1 | **真实 S3/MinIO + 跨机压测** | 把 `--cold-root` 从本地目录换成真实对象存储；跨机跑 §37 的同一套 | 里程碑用的是"共享目录"形态；**真实部署的延迟/失败语义只在 S3 上才真**（`§37` 的剩余） |
...
| `operation-log.md` | 实施日志 + **偏差与证据**（§1–§100） | ✅ 最新 | 每刀一节的纪律保持；§98（M4 并发真写）/ §99（读侧栅栏）/ §100（客户端落点 = contact point） |
