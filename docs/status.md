...
- **质量网**：355 用例全绿（+1 ignored） / clippy 0 警告（**全仓**，非热缓存读数）/ chaos **11/11** / T8 基线已入库 / 五个真缺陷已抓出（4 修 1 待）。
...
规模：**44,532 行 Rust / 20 个 crate / 355 个测试函数 / 355 个用例通过（+1 ignored） / clippy 0 警告**
...
| 层 | 证据 | 位置 |
|---|---|---|
| 单元 / 集成 | **355 passed / 0 failed**；355 个测试函数；`clippy --workspace --all-targets` 0 警告（本仓） | `cargo test --workspace` |
