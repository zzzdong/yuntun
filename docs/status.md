...
- **质量网**：349 用例全绿（+1 ignored） / clippy 0 警告（**全仓**，非热缓存读数）/ chaos **11/11** / T8 基线已入库 / 五个真缺陷已抓出（4 修 1 待）。
...
规模：**43,687 行 Rust / 21 个 crate / 349 个测试函数 / 349 个用例通过（+1 ignored） / clippy 0 警告**
工具链：**rustc 1.98.1 + edition 2024**（21 个 crate 全走 `edition.workspace = true`；见 `operation-log §59`）；
...
| 层 | 证据 | 位置 |
|---|---|---|
| 单元 / 集成 | **349 passed / 0 failed**；349 个测试函数；`clippy --workspace --all-targets` 0 警告（本仓） | `cargo test --workspace` |
