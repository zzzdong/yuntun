...
- **质量网**：377 用例全绿（0 ignored） / clippy 本仓 0 告警 / chaos **11/11** / T8 基线已入库 /
...
- **真实对象存储 / 跨机**：**本机 SeaweedFS（S3 兼容）的读写两向已验**（flush → HTTP PUT 可见于桶；
  删 WAL 重启后冷读 → HTTP GET；`scripts/s3_smoke.sh` + `§101`）；**真云 S3（TLS/签名/限流）
  与跨机 / 跨可用区仍未测**。
...
| # | 事项 | 准入 / 验收 | 为什么是这个顺序 |
|---|---|---|---|
| 1 | **真实 S3/MinIO + 跨机压测** | 本机 S3 **读写已验**（`§101` / `scripts/s3_smoke.sh`）；**剩余**：真云 S3（TLS / 签名 / 限流 / 503 退避）+ **跨机**跑 §37 的同一套 | 里程碑用的是"共享目录"形态；真实部署的延迟/失败语义只在**真 S3 + 跨机**上才真 |
...
| `operation-log.md` | 实施日志 + **偏差与证据**（§1–§101） | ✅ 最新 | 每刀一节的纪律保持；§98（M4 并发真写）/ §99（读侧栅栏）/ §100（客户端落点）/ §101（真实 S3 网络读写验证） |
