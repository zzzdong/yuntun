...
- **质量网**：333 用例全绿（+1 ignored） / clippy 0 警告（**全仓**，非热缓存读数）/ chaos **11/11** / T8 基线已入库 / 五个真缺陷已抓出（4 修 1 待）。
...
规模：**41,213 行 Rust / 20 个 crate / 333 个测试函数 / 333 个用例通过（+1 ignored） / clippy 0 警告**
...
| 层 | 证据 | 位置 |
|---|---|---|
| 单元 / 集成 | **333 passed / 0 failed**；333 个测试函数；`clippy --workspace --all-targets` 0 警告（本仓） | `cargo test --workspace` |
