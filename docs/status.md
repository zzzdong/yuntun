...
- **对外口径**：单机可实际使用；多节点**已可写、可压、可 GC，且不重不漏**（`§91`/`§98`/`§99`）。
  但**工程化项仍在**：客户端"给谁"未接、trust 无鉴权、真实 S3/MinIO 与跨机未验（见 §5.2 / §6）。
...
| 单写者多读 | ✅ | 上一行的子情形 |
| **客户端"给谁"** | ⚠️ 手工 | 设计说"无路由 ⇒ 任选一个数据进程"（`architecture §5`），但**"任选"的依据（名录 + 存活）还没接进客户端**；当前由调用方显式指定地址（`§95.4`） |
...
- **客户端"给谁"**：设计说"无路由 ⇒ 任选一个数据进程"，但落脚点仍靠**人工指定地址**
  （`§95.4`）。名录里只有**数据面**地址、没有 SQL 面地址 ⇒ 客户端无法直接据名录选落点。
...
| # | 事项 | 准入 / 验收 | 为什么是这个顺序 |
|---|---|---|---|
| 1 | **客户端"给谁"**：把"任选一个数据进程"（名录 + 存活）接进客户端 | 客户端只需一个 metanode 地址即可写入；名录提供可连的落点（正路：给名录**加 SQL 面地址**，或客户端先问名录再选活着的 ingest 节点） | 它是 `architecture §5`"无路由"承诺的**最后一环**；`§95.4` 已把三条路摆清楚 |
| 2 | **真实 S3/MinIO + 跨机压测** | 把 `--cold-root` 从本地目录换成真实对象存储；跨机跑 §37 的同一套 | 里程碑用的是"共享目录"形态；**真实部署的延迟/失败语义只在 S3 上才真**（`§37` 的剩余） |
| 3 | **冷 parquet 读纳入热读预算**（`§88.6`） | 大冷扫描也要有可配的上界；超预算 ⇒ 明确失败/标记，而不是无限等 | 现在预算只盖热读，冷读那段是**同一次查询的另一半** |
| 4 | **鉴权 / 成员变更（S3-6）** | metanode 成员变更（learner→voter）+ 端口鉴权；部署文档写明"trust 需网络隔离" | 多节点上生产的前提；同时是"稳定触发快照安装"的正路（`§42.4b`） |
| 5 | 同分片**在途写入**的细粒度时序断言（`§98.4`） | 对"交错提交的线性化点"下断言（现在只断言最终结果相等） | 属"更强"，非"缺失" |
| 6 | `known_batch_ids` 粒度（`§99.4`） | 按分片细分或用摘要替代全量集合；大表也要低开销 | 正确性已够；这是**规模**问题 |
| 7 | 文档/口径清理：`design.md §11 [chunk]` 重写、`README §4` 与代码的 Known limitations 对齐、`refactor.md` 状态列 | 逐条对照现状 | `status.md` 是现状入口，上游文档不能比它旧（§8 规则 3） |
| 8 | MSRV `1.94` 真编译验证（`§60`）；chaos 去 flaky（`plan` T6.1） | 用 1.94 真构建一次；chaos 独立跑或放宽 gate | 声明了就要验；flaky 会掩盖真缺陷（`§28.1` 就是这么被抓出来的） |
| 9 | MySQL 结果集 warning（`§89.4`） | 上游 `opensrv` 放开后再接 | **被依赖卡住**，不占本仓带宽 |
...
| `operation-log.md` | 实施日志 + **偏差与证据**（§1–§99） | ✅ 最新 | 每刀一节的纪律保持；§98（M4 并发真写）/ §99（读侧栅栏） |
| `plan.md` | 开发计划任务书（阶段 WBS / 里程碑 / 风险） | ✅ 已同步 | §8.4 门槛账更新为 M2–M6 全 ✅（引用 §98/§99）；§2.1/§七 缺陷状态同步 |
| `architecture.md` | 架构设计（12 个 ADR + 域设计） | ✅ 已修版本漂移 | v12：ADR-10 正式修订（含"≤1 文件/窗口"的限定语）；**欠**：§4 ADR-9 的 RPO 口径表述 |
| `design.md` | 详细设计（模块/接口/状态机/配置） | ⚠️ 部分章节是阶段 0 意图，实现有偏差 | 已声明"实现偏差以 operation-log 为准"；**欠**：§11 `[chunk]` 段与 `idle_timeout` 整段重写 |
| `refactor.md` | 分布式改造指南（S0–S6 / R0–R6 怎么改） | ✅ S1–S6 主要项已落地（R2–R6，见 `operation-log §38–§99`），其余仍是改造指南 | 状态列待逐条回填 |
