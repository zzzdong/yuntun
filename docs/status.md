...
- **多节点验到哪一层**（近期几刀，逐层加硬）：**3 个真 metanode 进程**换主不丢 + 进程级重启追平
...
- **质量网**：388 用例全绿（**0 ignored**） / clippy 本仓 0 告警 / chaos **11/11** / T8 基线已入库 /
  ...
  **不在 raft 而在传输层出站队列的队头阻塞** —— 过时的心跳/空追加堵住队头（队头还要等满 2s 超时），
  唯一携带条目的那条 append **一次都没出去**（`§110` 实测：交出 537 条 / 出队 298 条 / 到达 0 条）；
  修法是出站分**两条队列**（重要 / 可取代）+ **优先发携带条目的**（`§111`）。原复现探针按纪律
  **拿掉 `#[ignore]`、转正为回归用例** ⇒ **全量 0 ignored**。
  诊断配方仍是 `RUST_LOG=raft=debug` + `YUNTUN_META_TRACE=1`（`§106.1` / `§107.1`）。
- **部署包线（`§113.4`，量过的）**：`connect_timeout=500ms` / `RPC_TIMEOUT=2s` / raft 选举超时 ~1s
  量级 —— **对称延迟到 1.5s 就会反复选举**（term 猛涨），跨可用区 / 跨机部署前这三个都得按 RTT 重定。
- **对外口径**：单机可实际使用；多节点**已可写、可压、可 GC，且不重不漏**（`§91`/`§98`/`§99`）。
  客户端落点**由部署显式给定**（contact point，**设计如此**，D13/`§100`）；**工程化项仍在**：
  trust 无鉴权、**真云** S3（TLS/签名/限流）与**跨机**未验（真实 S3 的读写已验，见 `§101`/`§102`）。
  另两条**部署须知**：① `--snapshot-log-entries` 已有（`§105`）但**别调小**（`§105.3`）；
  ② 「落后节点靠快照追上」的**集成层稳定触发仍未拿到**（`plan T11.5` 第三项；正路是成员变更 `Join`）。
...
| M3 = R3 准出 | ✅ | 3 节点真 gRPC raft + 换主不丢已提交（`§50`）；**3 个真 metanode 进程**：换主不丢 + 进程级重启追平（`§103` —— 顺出并修掉"多节点真部署根本起不来"的两处缺陷）；metanode `kill -9` 后从盘恢复（`§48`）；**容器化三节点真集群 + 真断网**（`§108`：`tests/cluster.sh`，含"接回后必须收敛"硬断言）；standalone 接口行 + **二进制冒烟**（`§93`） |
| M4 = R4 准出 | ✅ | 多写者各出各的文件 + 共享冷存储 ⇒ **逐行精确相等**（`§91`）；**两个写进程并发真写**（`§98`）；**读侧栅栏**（`§99`） |
| M5 = R5 准出 | ✅（**部分**） | 扇出并发化 + 每查询热读预算（`§88`）；partial/STALE 语义（`§77`/`§78`）；结果不完整交给用户（`§89`）。**欠**：`partial aggregate 下推`（各节点算完再回传，而不是回传原始行）与 `冷数据按 datanode 分配`**未兑现** —— 数据面回传的是原始行（`shard.proto:67-70`），协调者只扇出**热**数据；登记在 `closeout.md` 台账 `D-4`（`§124`） |
...
规模：**51,547 行 Rust / 20 个 crate / 394 个测试函数 / 393 个用例通过（0 ignored） / clippy 0 警告**
...
| 层 | 证据 | 位置 |
|---|---|---|
| 单元 / 集成 | **393 passed / 0 failed / 0 ignored**；394 个测试函数；`clippy --workspace --all-targets` 0 警告（本仓） | `cargo test --workspace` |
...
| R3 快照安装（**遗留**，不阻塞里程碑） | **触发链条已闭合，但"稳定触发"刻意不做断言**：`§42.4b` 的 `installs=0 但已收敛` 已归因（根因是驱动层没有压缩触发）；`§105` 用 `--snapshot-log-entries` 在**单节点**上钉住"策略真的会触发"；**多节点**这边 `§109` 在**容器真集群**里**观测到**落后 follower 靠快照追平，`§111` 之后那条"小阈值 + 落后 follower"的回归用例**常绿**（`§122` 复核 5/5，并把压缩位打进日志：实测 `snapshot` 0→4→8→12 一路推进而写入全程被接受）。**刻意不写 `snapshot_installs >= 1` 这种断言** —— leader 是否发快照取决于它对该 peer 进度的判断（`raft_poc.rs` 实测过并据此撤掉了那条断言），写成断言会是**抖的** | `operation-log §42.4b` / `§42.7` / `§105` / `§109` / `§111` / `§122` |
...
| `operation-log.md` | 实施日志 + **偏差与证据**（§1–§124） | ✅ 最新 | 每刀一节的纪律保持；§98（M4 并发真写）/ §99（读侧栅栏）/ §100（客户端落点）/ §101（真实 S3 单机读写）/ §102（数据进程接 S3 + 多节点真 S3 对拍）/ §103（3 个真 metanode 进程 + 修掉多节点启动缺陷）/ §104（网络分区：少数派不能提交）/ §105（压缩触发策略）/ §106（写停摆复现）/ §107（写停摆**机制钉死**：乐观推进 + 丢一条 append；修法由 `§110`/`§111` 落地）/ §108（**容器化三节点真集群**：真断网 + 接回收敛断言，抓出并修掉"对端换 IP 后 leader 再也送不到"）/ §109（容器真集群检验 `§106`：**这一形态下没复现**，另拿到**快照安装**的正面证据）/ §110（续查：首因是**传输队列的队头阻塞**）/ §111（**写停摆修好**：出站分两条队列 + 优先发携带条目的，探针转正 ⇒ **0 ignored**）/ §112（`tests/cluster.sh soak`：反复冻结/断网下的收敛回归）/ §113（有损/延迟链路 + **部署包线**：对称延迟 1.5s 就反复选举）/ §114（`T8.x`「R3 后打同一 Meta」：**raft 元数据的代价** p99 +14.5%）/ §115（**3 节点** raft 元数据：代价主体是**串行化**，峰值提交 34→23→13）/ §116（把 **RTT** 加进去：跨区 30ms ⇒ p99 +39.7%、**峰值提交塌到 7 次/秒**）/ §117（netem 做满：抖动/丢包/乱序组合）/ §118（`Join` 上半刀：在线加 learner + 地址随 conf change 复制）/ §119（`Join` 下半刀：新节点 `--join` 在线加入并追平 + 地址表落盘）/ §120（提升 learner→voter + 两个被照出来的真问题：`Status.applied_index` 停摆、成员表顺序）/ §121（**移除成员**：`Meta.Remove` + 最后一个 voter 的硬门槛 + 进程级"4 voter 对照实验"）/ §122（对外口径校正：写停摆已修而文档仍写"未修"）/ §123（**偏差审计 + 判据化**：五类差异 + `closeout.md` 台账 + **5 条文档一致性判据**）/ §124（**台账逐条闭环**：`D-2` 编解码与 concat 下阻塞池（`YUNTUN_FORMAT_TRACE=1` 可观测）、`D-3`/`D-4` 定案、`D-1` 决策备忘） |
