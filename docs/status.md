...
- **质量网**：312 用例全绿 / clippy 0 警告（**全仓**，非热缓存读数）/ chaos **11/11** / T8 基线已入库 / 五个真缺陷已抓出（4 修 1 待）。
...
规模：**38,302 行 Rust / 18 个 crate / 312 个测试函数 / 312 个用例通过 / clippy 0 警告**
...
| 层 | 证据 | 位置 |
|---|---|---|
| 单元 / 集成 | **312 passed / 0 failed**；312 个测试函数；`clippy --workspace --all-targets` 0 警告（本仓） | `cargo test --workspace` |
...
| 4 | **R4：datanode 化 + 冷热边界**（**进行中**） | M4：多 datanode 并发写 + 查询结果**与单节点串行精确相等**（对拍，硬要求） | 这一步才消费 `source_instance` → 消灭缺口 §5.1-1（重复计数）。**已落地**：① **P2 闸门已过** —— 冷热边界协议形态定为**拉取**（水位随 pull 响应回，推送仅可作提示），契约 4 条定死，含两种形态的查询放大对比（`§61`）；② **T12.4/T12.5 私有目录闸门** —— WAL 根目录 + spill 目录的排他所有权，同一目录第二个消费者**启动即被拒**（`§62`，std `File::try_lock`，零 unsafe/零新依赖）。**下一步**：T12.2 的三刀（`ShardReader` 加水位与 STALE → 查询侧按实例持有热读器 → 双实例进程内对拍） |
