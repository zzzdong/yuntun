...
规模：**37,578 行 Rust / 18 个 crate / 302 个测试函数 / 302 个用例通过 / clippy 0 警告**
（本次改动 crate；全仓余 1 处历史告警 `chunk/store.rs`）。
