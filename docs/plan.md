...
| **P1** 🆕 | ~~内存水位是第五个 seal 触发器~~ → **seal 原因不可观测**：高吞吐下每窗口每 shard 26~92 文件、`seal→committed` 28.6~49.3s（承诺 30s），但**为什么**封口在 27000 行未证实（阈值解释不了；压力是候选，`operation-log §34.2` 显示让位逻辑在该负载下未触发） | 已实现"相位让位"（水位 ≥ Soft 时已 sealed 的立即 flush，避免 Hard 强封 open chunk + spill），计数可观测 | `operation-log §33/§34`：低速 ✅（1 文件/窗口、9.8s）；高速 ❌（26~92 文件/窗口、28.6~49.3s）；`spread` 只影响延迟与档位、**不影响文件大小** | **R4 前**：先做 seal 原因观测（§34.3），再按原因处置 |
