...
| 阶段 2 | R0 收尾、R1 收尾 | 质量与性能：Chaos 11 场景 + 压测 baseline + Vortex + **观测指标**；**本阶段同时承担 §2.2 的定案**（阶段 3 的准入门槛） | 部分：观测已落地（T6.12 ✅）；chaos 已到 7/11（§27 / §28.2 已修，§28.1 待与 R3 同批，**§29.1 WAL 撕裂后不可自愈待修**）；压测/定案待做 |
...
| # | 决策项 | 现状 | 定案所需证据 | 期限 |
|---|---|---|---|---|
| **P0** | `flush_phase_spread_secs` 量级（5s → 60s？）与 `max_flush_delay_secs`（30s → 0？） | 5s / 30s。窗口对齐后低吞吐表 flush 集中在"窗口关闭 + 30~35s"的 5s 带内，**分散面比 ADR-10 收窄 12 倍** | 阶段 2 baseline：同表多 shard 下的 CommitFiles / S3 PUT 瞬时并发峰值；文件数/天；持久化 P99 | 阶段 2 首轮 |
...
| # | 项 | 目标文档 | 期限 |
|---|---|---|---|
| 1 | **ADR-10 正式修订**：jitter 锚点由 `window_start` 改为 `seal_time`；量级待 P0 定案；保留"窗口对齐 + 每窗口每 shard ≤1 文件"的目标 | `architecture.md` §4 | 阶段 3 前（与 R2 Catalog 冻结同批） |
...
| 基线压测 | 未入库 | **P0 定案（相位分散量级 / `max_flush_delay` / `rows_threshold`）没有数据支撑** | 阶段 2 |
...
| ID | 内容 | 说明 | 为什么现在做 |
|---|---|---|---|
| T6.1–6.11 | Chaos 11 场景（v1.0 §5.2 全表沿用） | 含并发崩溃、`synced_seq` 验证、Batch 超时、磁盘水位 | 阶段 3 前置：`refactor.md §3` 明文"不要跳过"。**进行中：chaos 层 7/11（#1–#6、#9）；#7 抓到缺陷待修；#8/#10/#11 待做**。已抓出四个真缺陷：幂等键不生效（§27 ✅）、恢复重复文件（§28.2 ✅）、提交窗口重复计数（§28.1，待与 R3 同批）、**WAL 撕裂后无法自愈（§29.1，最严重）** |
...
| T8.x | 压测与基线入库 | 吞吐 / P99 / 内存曲线 / **CommitFiles 瞬时并发** / 文件数·天 | P0 的证据来源 |
...
| **R-14** | **WAL 撕裂后无法自愈**：`open_append` 以文件物理长度作追加偏移，新记录写在撕裂的垃圾字节之后，而 replay 扫到撕裂点即停止 → **写入全部成功（ack 正常）但数据全部不可见**，静默、永久 | **已发生** | **高** | 修复：恢复时把 segment **截断到最后一条完整记录边界**（`set_len`），再让 writer 从该偏移续写（WAL 标准修复步骤）。`decode_segment_records` 已给出停止位置，只是没人回写。用例 `torn_wal_tail_is_rejected_and_prefix_survives` 已就绪（`#[ignore]`），修完去掉即可 —— 见 `operation-log §29.1` |
...
| 3 | **P0 三项全部定案**，默认值已按定案调整，ADR-10 已修订 | §2.2 登记表逐项签署 |
