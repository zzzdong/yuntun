...
| T12.3 | 成员发现 + 分片归属（谁持有哪个 `(table, shard)` 的热数据） | 与 `ShardReader`/`ShardFetch` 对接（缝已在）。**第一刀已落地**（`§69`）：**成员表作为唯一真相** —— 快照 `nodes` 与热读器键集都由它派生（关掉 `§65.5`"同源但未强制一致"的坑），成员带**数据面地址**，摘除成员即移除其热读器（可观测）。**未做**：向 metanode 注册/心跳/超时摘除（`§3.2` 两层机制）、按归属收窄拉取范围（属 R5 fanout T13.1） |
