...
| T11.5 | 3 节点 raft：杀 leader / 网络分区 / snapshot 重建 | 阶段 3 的 chaos；**真进程**杀 leader（`§103`）+ **网络分区**（`§104`：可切断链路、零生产改动、含脑裂防线与常驻反证）+ **压缩触发策略**（`§105`：`--snapshot-log-entries`（默认 10 万），补上"快照从没被触发过"那一环）已做；**snapshot 重建的集成层稳定触发仍未拿到**（`§105.3` 记了一个未根因的写停摆；正路是成员变更/`Join`） |
