...
**门槛审计（`§90` / 复评 `§91`，2026-09-24）**：M2 ✅ / **M3 ⚠️**（③ **standalone 真启动**缺证据）/
**M4 ✅**（`§91` 已补上"多 datanode 并发写"的证据：两个真写进程 + 共享 `--cold-root` + 只查询
协调者 ⇒ 逐行精确相等）/ M5 ✅ / M6 ✅。
⇒ **仍不到"分布式就绪"**：只差 **M3 ③** 一格。该格的处置见 `§92`：
**装配层已有证据**（standalone 只有 96 行 = `Config → Lakehouse → serve_flight`，
与 `flight_e2e`/`assembly_parity` 是**同一个装配** ⇒ 覆盖合法转移）；
**二进制层冒烟未做** —— 且它不该"顺手补"：standalone 的地址来自 TOML，测试要么选固定端口、
要么先 bind 再释放（`§37` 批评过的 TOCTOU）⇒ 干净做法是**先给 `yuntun` 加一行 `LISTEN <addr>` 接口行**。
