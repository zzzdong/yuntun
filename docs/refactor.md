...
> 📌 **详细设计与分步计划已定稿**（2026-09-19）：[`metanode-design.md`](metanode-design.md) ——
> 含 raft 库选型（raft-rs + 逃生门）、`CatalogState` 抽取（R3 最大重构）、快照与保留策略、
> proto 草案、S3-0~S3-7 步骤与回滚点、M3 验收矩阵与 8 条风险。
> 本节的 S3-1~S3-10 是原始清单，**以设计文档为准**。
