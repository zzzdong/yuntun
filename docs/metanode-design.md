...
| **S3-4** | `RemoteCatalog`（`CatalogOps` 的 gRPC 实现）+ standalone 装配（本地传输、1 节点 raft） | **第一件已落地**（`operation-log §51`）：**读路径载荷定形**（`PrefetchPayload` + `TableMeta` 镜像；四条语义分支逐条有用例 + 反证；抓到「载荷名必须全限定」这个静默串表的坑）。**余**：`RemoteCatalog` + standalone 装配 → 判据仍是「既有用例全绿 + `if distributed` 分支为零」 | 切回 `MemoryCatalog`（装配层开关） |
