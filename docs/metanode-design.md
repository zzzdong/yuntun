...
| **S3-4** | `RemoteCatalog`（`CatalogOps` 的 gRPC 实现）+ standalone 装配（本地传输、1 节点 raft） | **第一件已落地**（`operation-log §51`）：**读路径载荷定形**（`PrefetchPayload` + `TableMeta` 镜像；四条语义分支逐条有用例 + 反证；抓到「载荷名必须全限定」这个静默串表的坑）。**`RemoteCatalog` 已落地、与 `MemoryCatalog` 对拍一致**（`§55`）；**余**：**只剩「切装配点」**判据仍是「既有用例全绿 + `if distributed` 分支为零」。**文件级读载荷已补齐**（`§54`：增量含墓碑）。**其余 5 个 op 镜像已补齐**（`§53`：DropSchema/EvolveSchema/DropShard/Compaction/幂等认领；`Op.kind` 分支 4→9）。**接缝已拆**（`§52`）：`Lakehouse.catalog` 已是 `Arc<dyn CatalogOps>`，生产代码只剩装配点知道实现，且实测**没有「只有具体类型才有」的隐藏依赖**（21 个方法全在 trait 上） | 切回 `MemoryCatalog`（装配层开关） |
