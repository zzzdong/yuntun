...
| 步 | 内容 | 验收（必须能跑） | 回滚点 |
|---|---|---|---|
| **S3-0** | proto 定义 + `tonic-build`（`T10.8`）：把现有手写 prost struct 迁到 `.proto` 生成 | ✅ **第一切片已落地**（`operation-log §45`）：`Meta` 服务面（Propose/Prefetch/Delta/Status/Join）冻结；op 面已迁移 `CreateSchema`/`DropTable`/`CommitFiles`，**round-trip + 与 `CommitFilesRequest` 逐字段对齐**均通过（含反证）。**余**：其余 op 的镜像、`result`/`payload` 逐 op 定形 | ✅ 保留手写 struct（手写结构一个没动，接口面独立） |
...
| **S3-3** | `yuntun-meta` 进程 + `MetaService`（Propose/Prefetch/Delta/Status/Join）+ fjall | ✅ **服务层已落地**（`§47`）：`MetaService` + `NodeHandle` + **真 gRPC 端到端用例**（写入/幂等重试/Status/Delta/UNIMPLEMENTED）；**余**：CLI/`--init` bootstrap、多节点部署形态、其余 op 镜像、Prefetch（S3-4）、Join（S3-6）。**op 生产路径已接线（`§46`）**：proto `Op` 成为唯一权威编码（日志 payload 与 gRPC 面同构）、边界转换器双向逐字段、错误码映射（约定 3）落地、集群用例已跑生产路径。单节点 metanode 可独立启动；重启后状态一致。**进行中（2026-09-19）**：**gRPC 面（proto）已落地**（`§45`）、**落盘 `Storage`（fjall）已完成**、**节点已切到落盘版且 M3 的 G1 判据在 PoC 层通过**（`§43`/`§44`）—— 先盘后缓存 + **原子批（把"产物↔压缩位置"不变量交给存储保证）** + fsync 分级；5 条测试含 3 条跨 reopen（`operation-log §43`）。**余**：节点装配（把 PoC 从内存版切成 fjall 版）、两类时钟统一、快照触发/保留策略、真崩溃（子进程）注入 | — |
| **S3-4** | `RemoteCatalog`（`CatalogOps` 的 gRPC 实现）+ standalone 装配（本地传输、1 节点 raft） | **第一件已落地**（`operation-log §51`）：**读路径载荷定形**（`PrefetchPayload` + `TableMeta` 镜像；四条语义分支逐条有用例 + 反证；抓到「载荷名必须全限定」这个静默串表的坑）。**`RemoteCatalog` 已落地、与 `MemoryCatalog` 对拍一致**（`§55`）；**已完成**（`§55`：`RemoteCatalog` 与 `MemoryCatalog` 对拍一致；`§56`：**装配点已切换**，回滚开关 `[meta] mode = "memory"`；整机对拍等收尾项见 `§56.4`）判据仍是「既有用例全绿 + `if distributed` 分支为零」。**文件级读载荷已补齐**（`§54`：增量含墓碑）。**其余 5 个 op 镜像已补齐**（`§53`：DropSchema/EvolveSchema/DropShard/Compaction/幂等认领；`Op.kind` 分支 4→9）。**接缝已拆**（`§52`）：`Lakehouse.catalog` 已是 `Arc<dyn CatalogOps>`，生产代码只剩装配点知道实现，且实测**没有「只有具体类型才有」的隐藏依赖**（21 个方法全在 trait 上） | 切回 `MemoryCatalog`（装配层开关） |
