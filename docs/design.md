...
> **依据**：《通用直写数据湖架构设计 v12》（`architecture.md`）
> **版本**：v1.2
> **日期**：2026-08-31（v1.1：2026-09-08；**v1.2：2026-09-18 —— 指向 `status.md`，并明确"实现偏差以 `operation-log` 为准"**）
>
> ⚠️ **时效声明**：本文是**设计意图**（阶段 0/1 定型）。自 S1（chunk 层）以来实现有多处**已登记偏差**
> （详见 `operation-log §25/§27/§30/§32`），凡与本文不符处，**以 `operation-log` + [`status.md`](status.md) 为准**，
> 并应回改本文对应段落（未回改的即视为欠账，见 `status.md §7`）。
